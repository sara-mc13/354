...
// Instructor:       deppeler
//
// Author:           Praveen Sundaram
// Email:            psundaram@wisc.edu
// CS Login:         praveen
//
...
int valid_board(int **board, int size) {
    //checks to see if values are between 1 and size
    ...
    for (int i = 0; i < size; i++) {
        int *rowVals = malloc(sizeof(int) * (size));
        for (int j = 0; j < size; j++) {
            if (*(*(board + i) + j) > 0) {
                if ((*(rowVals + (*(*(board + i) + j))))) {
                    return 0;
                } else {
                    *(rowVals + *(*(board + i) + j)) = 1;
                }
    ...
    for (int i = 0; i < size; i++) {
        int *colVals = malloc(sizeof(int) * (size));
        for (int j = 0; j < size; j++) {
            if (*(*(board + j) + i) > 0) {
                if ((*(colVals + (*(*(board + j) + i))))) {
                    return 0;
                } else {
                    *(colVals + (*(*(board + j) + i))) = 1;
                }
