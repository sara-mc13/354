...
void* balloc(int size) {     
    ...
    //adds padding (size + 4 = payload + header)
    size += sizeof(blockHeader);
    if (blockSize % 8 != 0) {
        blockSize += blockSize + (8 - (size_alloc % 8));
    }
    ...
    while (current->size_alloc != 1) {
        ...
        if (curr->size_status % 8 == 0 || curr->size_status % 8== 2) {
        ...
        if (current >= (blockHeader*)((char*)heap_start + alloc_size) {
            ...
            if (bestFit == NULL) {
                ...
                int sizeAfterAllocation = (bestFit->size_status & (~3)) - size;
                //if block big enough to split, split into two (one allocated and free)
                if (sizeAfterAllocation > 8) {
                    blockHeader *newHeader = (blockHeader*)((char*)bestFit + size);
                    blockHeader *newFooter = (blockHeader*)((char*)newHeader + sizeAfterAllocation - sizeof(blockHeader));
                    newHeader->size_status = sizeAfterAllocation + 2;   //previous allocated, current free
                    newFooter->size_status = sizeAfterAllocation;       //size of new block
...
int bfree(void *ptr) {    
    ...
    if ((int)ptr % 8 != 0) {    //Return -1 if ptr is not a multiple of 8.
        return -1;
    }

    blockHeader *block = ptr - sizeof(blockHeader);
    //Return -1 if ptr is outside of the heap space.
    if (block < heap_start || block >= (blockHeader*)((char*)heap_start + alloc_size)) {
        return -1;
    }
    ...
    //add footer
    blockHeader *footer = (blockHeader*)((char*)block + (block->size_status & (~3)) - 4);
    footer->size_status = block->size_status & (~3);
    //nextHeader
    blockHeader *nextHeader = (blockHeader*)((char*)footer + sizeof(blockHeader);
    //if next is in heap space, set p bit to 1
    if (nextHeader < (blockHeader*)((char*)heap_start + alloc_size) {
        nextHeader->size_status = nextHeader->size_status & (~2);
    }
    return 0;
...
int coalesce() {
    ...
    //while current in heap space
    while (current < (blockHeader*)((char*)heap_start + alloc_size) {
        blockHeader *nextHeader = (blockHeader*)((char*)currBlock + (currBlock->size_status & (~3)));
        if (nextHeader < ((blockHeader*)((char*)heap_start + alloc_size)) //next in heap space
        && ((current->size_status & 1) == 0) //current is free
        && ((nextHeader->size_status & 1) == 0)) //next is free {
            blockHeader *nextFooter = (blockHeader*)((char*)nextHeader + (current->size_status) & (~3) - 4);
            blockHeader *currentFooter = (blockHeader*)((char*)current + (current->size_status & (~3) - 4);
            currentFooter = nextFooter;
            currentFooter->size_status = (current->size_status & (~3)) + (nextHeader->size_status & (~3));
            current->size_status = currentFooter->size_status | 2;
        } else {
            currentBlock = (blockHeader*)((char*)current + (current->size_status & (~3));
        }
    }
	return 0; 
}
